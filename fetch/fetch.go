		...
		case response := <-collectorCh:
			...
			for f.chunkBuffer.Len() > 0 {
				...
				for blockIndex, block := range item.chunk.blocks {
					...
					if txNum > 0 {
						// tx success
						txErr := item.chunk.results[0][0].Response.ResponseBase.Error
						if txErr == nil {
							// iterate txs
								...
								for _, msg := range stdTx.Msgs {
									...
									if _type == "add_package" {
										...
										if isGRC20(funcNameList) {
											// register
											addpkg.RegisterGrc20Token(pkgPath)
										}
									}
								}
							}
...
func isGRC20(mainSlice []string) bool {
	// https://github.com/gnolang/gno/blob/0f2e7551b43c18d27b63cbbadecf07ee48f185f9/examples/gno.land/p/demo/grc/grc20/imustgrc20.gno#L13-L21
